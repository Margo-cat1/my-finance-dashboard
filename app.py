...
# --- БЛОК АВТОРИЗАЦИИ ---
# В реальной жизни эти данные лучше хранить в отдельном файле или БД
names = ['Admin User', 'Margo']
usernames = ['admin', 'margo']
# Хешированные пароли (генерация через stauth.Hasher)
# Пароль для теста: '12345'
passwords = ['abc123', 'your_password_here']
# ПРИМЕЧАНИЕ: Для теста ниже я использую простую логику,
# так как хеширование требует предварительного прогона через утилиту.

authenticator = stauth.Authenticate(
    {'usernames': {
        'admin': {'name': 'Admin User', 'password': '123'},  # В продакшене тут должен быть хеш!
...
if authentication_status == False:
    ...
    # --- ЕСЛИ ВХОД УСПЕШЕН, ПОКАЗЫВАЕМ ВЕСЬ ТВОЙ КОД ---
    ...
    LANGS = {
        ...
        "English": {
            ...
            "guide": {
                ...
                "sol2": "**Solvency 2:** Assets - Liabilities. **Minimum: > 0**.",
                "sol3": "**Solvency 3:** Net Assets / Total Assets. **Minimum: 50%**.",
                "qr": "**Quick Ratio:** Cash / Short-term Debt. **Minimum: 2.0**."
            }
        },
        "ქართული": {
