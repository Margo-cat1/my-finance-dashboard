...
# 2. Словарь переводов (Исправлены все ключи для всех языков)
LANGS = {
    "Русский": {
    ...
    "ქართული": {
        ...
        "cash": "ნაღდი ფული (Cash)", "ebitda": "EBITDA",
...
with tab1:
    ...
    with col_s:
        ...
        if sol3_pct > 50: st.write("🌟 **Автономия:** Большая часть активов — ваши.")
        if roi > 20: st.write(f"📈 **Окупаемость:** Высокий уровень ROI ({roi:.1f}%).")
        if qr >= 0.5: st.write("💧 **Ликвидность:** Хороший запас наличности.")
    with col_r:
        st.warning(f"### {t['risks']}")
        if sol3_pct < 30: st.write("🚩 **Зависимость:** Высокая доля долгов.")
        if qr < 0.2: st.write("💸 **Дефицит Cash:** Мало денег для быстрых платежей.")
        if sol2_val < 0: st.error("❌ **Банкротство:** Долги больше активов!")
